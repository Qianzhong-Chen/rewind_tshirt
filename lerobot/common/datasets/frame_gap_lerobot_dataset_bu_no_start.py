...
class FrameGapLeRobotDataset(LeRobotDataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        obs_indices = [idx - i * self.frame_gap for i in reversed(range(self.n_obs_steps))]
        ...
        # Progress targets
        progress_start = (obs_indices[0] - ep_start) / (ep_end - ep_start) if ep_end > ep_start else 0.0
    ...
    #     print(f"[TIMING] Total: {t8 - t0:.3f}s | "
    #         f"Preprocess: {t1 - t0:.3f}s | "
    #         f"Select: {t2 - t1:.3f}s | "
    #         f"Stack: {t3 - t2:.3f}s | "
    #         f"QueryVideo: {t4 - t3:.3f}s | "
    #         f"Rewind: {t5 - t4:.3f}s | "
    #         f"Transform: {t6 - t5:.3f}s | "
    #         f"TaskGen: {t7 - t6:.3f}s | "
    #         f"Progress: {t8 - t7:.3f}s")
