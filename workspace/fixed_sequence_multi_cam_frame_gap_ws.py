...
import wandb
from lerobot.common.datasets.frame_gap_lerobot_dataset import FrameGapLeRobotDataset 
from data_utils import comply_lerobot_batch, get_valid_episodes, split_train_eval_episodes
from train_utils import set_seed, save_ckpt, plot_pred_vs_gt
        ...
        for epoch in range(1, cfg.train.num_epochs + 1):
            ...
            for batch in pbar:
                ...
                with torch.no_grad():
                    ...
                    lang_emb = clip_encoder.encode_text(lang_strs)                # img_emb: (B, N, T, vis_dim), lang_emb: (B, txt_dim)
            ...
            if epoch % cfg.train.eval_every == 0:
                ...
                with torch.no_grad():
                    for batch in dataloader_val:
                        ...
                        pred = reward_model(img_emb, lang_emb, lens)
                        for i in range(B):
                            length = int(lens[i].item())
                            loss = F.l1_loss(pred[i, :length], batch["targets"][i, :length].to(self.device), reduction="mean")
                            total_loss += loss.item()
                        num += 1
                val_loss = total_loss / num / B
            ...
            if epoch % cfg.train.rollout_every == 0:
                ...
                with torch.no_grad():
                    for num, batch in enumerate(dataloader_rollout):
                        ...
                        plot_pred_vs_gt(pred[0], batch["targets"][0], result_dir / "plot.png")
                        rollout_loss += loss
                        num += 1
