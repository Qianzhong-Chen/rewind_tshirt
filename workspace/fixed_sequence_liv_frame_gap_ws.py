...
import wandb
from lerobot.common.datasets.liv_lerobot_dataset import LIVLeRobotDataset 
...
from raw_data_utils import get_frame_num, get_frame_data_fast_liv, get_traj_data, normalize_dense, normalize_sparse
...
class RewindRewardWorkspace:
    ...
    def compute_vip_loss(self, pred, b_reward, num_negatives=0, gamma=0.95, epsilon=1e-8):
        ...
        if num_negatives > 0:
            ...
            for _ in range(num_negatives):
                ...
                V_s_neg.append(F.mse_loss(es0_vip_shuf, eg))
                V_s_next_neg.append(F.mse_loss(es1_vip_shuf, eg))

            V_s_neg = torch.cat(V_s_neg)
            V_s_next_neg = torch.cat(V_s_next_neg)
            r_neg = -torch.ones(V_s_neg.shape).to(V_0.device)
            V_s_neg = V_s_neg / (1-gamma)
            V_s_next_neg = V_s_next_neg / (1-gamma)
    ...
    def eval_video(self):
        ...
        dataset_val = LIVLeRobotDataset(repo_id=repo_id, 
                                        episodes=valid_episodes, 
                                        image_names=cfg.general.camera_names,
                                        dense_annotation=cfg.model.dense_annotation,
                                        annotation_list=cfg.model.annotation_list,
        ...
        for i in range(cfg.eval.video_run_times):
            ...
            for idx in tqdm(range(start_idx, end_idx, eval_frame_gap), desc=f"Processing episode {ep_index}"):
                ...
                batch = comply_lerobot_batch_regression_eval(data_point, 
                                                        camera_names=cfg.general.camera_names, 
                                                        dense_annotation=cfg.model.dense_annotation)
                B, T = batch["image_frames"][self.camera_names[0]].shape[:2]
                img_list = []
                ...
                gt_ep_result.append(normalize_sparse(trg[0, 1].item()))
