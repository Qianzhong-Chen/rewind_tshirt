...
from models.multi_stage_residual_estimate_net import StageTransformer
from models.soft_classification import gaussian_soft_labels, masked_soft_ce, unimodal_penalty_from_logits
        ...
        for epoch in range(1, cfg.train.num_epochs + 1):
            ...
            with tqdm(dataloader_train, desc=f"Epoch {epoch}") as pbar:
                for batch in pbar:
                    ...
                    # Forward
                    stage_logits = stage_model(img_emb, lang_emb, state, lens)  # (B, T, K)
                    y_cont = (trg.clamp(0, 1) * (cfg.model.num_classes - 1)).to(stage_logits.dtype)   # (B, T)
                    soft_targets = gaussian_soft_labels(y_cont, K=cfg.model.num_classes, sigma=cfg.model.ordinal_sigma) # (B, T, K)
                    # Loss (pure classification, ordinal-aware)
                    ce_loss = masked_soft_ce(stage_logits, soft_targets, lens) 
                    uni_loss = unimodal_penalty_from_logits(stage_logits, lens)
                    stage_loss = ce_loss + cfg.model.unimodal_lambda * uni_loss
                    
                    # ===== metrics (masked) =====
                    with torch.no_grad():
                        probs = torch.softmax(stage_logits, dim=-1)          # (B, T, K)
                        pred_cls = probs.argmax(dim=-1)                      # (B, T)
                        # integer bins for accuracy
                        y_bins = torch.clamp((trg * cfg.model.num_classes).round().long(), max=cfg.model.num_classes-1)  # (B, T)
                        mask = (torch.arange(T, device=stage_logits.device)[None, :] < lens[:, None])
                        correct = (pred_cls == y_bins) & mask
                        top1_acc = correct.sum().float() / mask.sum().clamp_min(1)
                        # expected value MAE (just for monitoring)
                        classes = torch.arange(cfg.model.num_classes, device=stage_logits.device, dtype=probs.dtype)
                        ev = (probs * classes.view(1,1,-1)).sum(dim=-1)      # (B, T)
                        mae_ev = (ev - y_cont).abs()
                        mae_ev = (mae_ev * mask).sum() / mask.sum().clamp_min(1)
                        # calibration-ish: mean entropy
                        ent = -(probs.clamp_min(1e-12) * probs.clamp_min(1e-12).log()).sum(dim=-1)
                        mean_entropy = (ent * mask).sum() / mask.sum().clamp_min(1)
                    ...
                    if step % cfg.train.log_every == 0:
                        wandb.log({
                            ...
                            "train/stage_ce_loss": ce_loss.item(),
                            "train/stage_uni_loss": uni_loss.item(),
                            "train/stage_grad_norm": stage_unclipped,
                            "train/stage_top1_acc": top1_acc.item(),
                            "train/stage_ev_mae": mae_ev.item(),
                            "train/stage_mean_entropy": mean_entropy.item(),
                            "epoch": epoch,
                        }, step=step)
